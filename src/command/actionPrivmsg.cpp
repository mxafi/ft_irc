...
void Command::actionPrivmsg(Client& client) {
  ...
  if (channelPattern.find(targetParam.front()) != std::string::npos) {
    ...
    try {
      ...
      if (channel.isMember(client) == false) {
        client.appendToSendBuffer(
            RPL_ERR_NOTONCHANNEL_442(serverHostname_g, targetParam));
        return;
      }
      ...
      client.appendToSendBuffer(
          RPL_ERR_NOSUCHNICK_401(serverHostname_g, targetParam));
    }
    return;
  }
  ...
  try {
    ...
    LOG_DEBUG("CMD::PRIVMSG: Message is :" + messageParamWithoutColon +
              " from " + client.getNickname() + " to " + targetParam);

    std::string formattedSender = FORMAT_NICK_USER_HOST(
        client.getNickname(), client.getUserName(), client.getHost());
    std::string privmsg =
        PRIVMSG_FORMAT(formattedSender, targetParam, messageParamWithoutColon);
